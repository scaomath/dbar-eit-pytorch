...
%
% Samuli Siltanen March 2013, Adjusted: Andreas Hauptmann 2017
...
Fpsi_BIE = zeros(length(Nvec),length(Kvec));

R    = 7;
[K,indq1,indq2,indq3,indq4,kvecq1,kvecq2,kvecq3,kvecq4] = k_grid_creator(R+0.1,72);
...
for kkk = 1:length(Kvec)
    ...
    for jjj = 1:length(Ntrig)
        if jjj<=(nBasis+1)/2
            Fcald(jjj) = 1/sqrt(pi)*Dtheta*(cos(Ntrig(jjj)*theta))'*cald;
        else
            Fcald(jjj) = 1/sqrt(pi)*Dtheta*(sin(Ntrig(jjj)*theta))'*cald;
        end
    end
    
    % Construct the operator to be inverted
    Hk=get_Hk_KIT4(k,kkk,K);
...
save data/psi_BIE Fpsi_BIE Nvec 
